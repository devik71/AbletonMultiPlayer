...
**Статус: MVP працює на двох машинах.** Перевірено на парі Live 12.3.8 / 12.3.5
у локальній мережі: transport, tempo, clip launch/stop, scene launch, stop all,
створення й видалення треків і сцен — в обидва боки. Ідентичність об'єктів
піднімається з `.als`, тож обидві машини бачать однакові uuid без узгодження
по мережі. Семпли синхронізуються автоматично.

Не зроблено з MVP-скоупу vision.md §10: **локи на об'єкт** (одночасна зміна одного
параметра = виграє останній за `global_seq`). Audio monitor відкладений навмисно.
