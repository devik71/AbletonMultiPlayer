...
регіон нот) доїжджає остання. Той самий механізм стискає й журнал на диску, коли він переростає
`MP_COMPACT_AT` подій (2000): повна історія переїжджає в
`relay/journals/<session>.archive.jsonl`, який ніхто ніколи не обрізає.
`MP_COMPACT_JOIN=0` вимикає стиснення хвоста, `MP_COMPACT_AT=0` — журналу. Скільки подій зекономлено, видно в `/health`
...
кожен author, останню подію, розподіл подій за типами, хто що редагує
і хто на що дивиться просто зараз. Для віддаленого relay
...
Відомий розсинхрон. Натомість є власний undo: команда `undo [author]` у daemon
відкочує останню зміну — свою або партнерову — новою подією в журналі, тож
історія лишається цілою, а скасування видно так само, як і саму зміну.
...
Зворотний бік теж є: команда `apply` вирівнює твій сет по чужому знімку —
темп, назви, кольори, мікшер, усі параметри девайсів і ноти Session-кліпів.
Структуру знімок не чіпає: треки, сцени й девайси не створюються і не
видаляються, бо ціна помилки тут несиметрична — натомість daemon друкує список,
чого саме бракує («немає девайса Serum на треку Bass»), і кладе його повністю
у `daemon/state/<author>.<session>.missing.json`. Застосування не породжує подій —
...
`follow <author>|off` і `who` (присутність партнерів), `undo [author]`,
...
Поруч із локами працює **присутність**: у лозі daemon і в M4L Status видно,
на що дивиться партнер — трек, сцена, кліп. Команда `follow <author>` веде
твій вид за ним; вона призупиняється, щойно ти клацнеш сам, і мовчить, поки
транспорт грає з озброєним треком — бо виділений трек вирішує, куди йде MIDI.
