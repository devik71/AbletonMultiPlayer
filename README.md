...
- **`TrackCreate` / `TrackDelete`.** Реєстр будується один раз на бутстрапі.
  Додавання треку під час сесії залишає партнера зі старим реєстром — bridge
  пише warning, але нової адреси партнер не дізнається.
