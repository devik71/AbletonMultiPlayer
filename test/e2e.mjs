...
try {
  ...
  await check('watch сам знаходить розходження і мовчить про решту', async () => {
    ...
    // Рядки розбіжностей друкуються слідом за заголовком, але в потік до нас
    // потрапляють окремою порцією -- чекаємо саме на назву, а не на заголовок.
    await waitFor(d2, /WatchDrift/, 10000, from);
