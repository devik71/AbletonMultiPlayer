...
Це ще не compaction журналу з vision.md §5 п.7: relay нічого не обрізає й не
робить snapshot стану Live. Для цього bridge має вміти віддати повний стан
сету і застосувати його цілком (`snapshot_apply`) -- сьогодні `{"m":"snapshot"}`
з bridge діагностичний і містить лише tempo, transport, треки й сцени.
